...
namespace wkttool{
  ...
  template <typename T>
  struct VariantSignal {};

  template <typename... Ts>
  class VariantSignal <std::variant<Ts...>> : 
    boost::signals2::signal<void(const Ts&)>... {
    public:
      template <typename T> 
      boost::signals2::connection connect(T listener) {
        namespace ct = boost::callable_traits;
        return boost::signals2::signal<
          ct::function_type_t<T>>::connect(listener); 
      }
      template <typename T> 
      void operator()(const T& arg) {
        using Compatible = typename CompatibleType<T,InnerTypeWrapper<Ts...>>::Type;
